...
Use `NoSpaceError` as the scheduled exception type for disk-full scenarios. It
sets `errno.ENOSPC` while retaining the scenario and point text generated by
`FaultInjector`; tests should assert both the errno behavior and the protected
filesystem or queue state.
